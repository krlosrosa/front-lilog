import type { ConvertXlsx } from '../infra/convertXlsxRepository';
import {
  entregaSchema,
  produtoSchema,
  rotaSchema,
...
export class VerificarAnomalia {
  constructor(private readonly fileConverter: ConvertXlsx) {}
  async execute(input: FilesState): Promise<ErrorFiles> {
    ...
    const unicos = shipments.filter((shipment) => {
