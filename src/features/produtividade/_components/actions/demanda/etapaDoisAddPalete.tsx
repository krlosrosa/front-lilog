...
import { AddFuncionario } from "@/features/user/_views/addFuncionario";
import { useAuthStore } from "@/_shared/stores/auth.store";

type Funcionario = ListarFuncionariosPorCentroZodDtoOutput[number]

type EtapaDoisAddPaleteProps = {
  funcionarioSelecionado: Funcionario | null;
  setFuncionarioSelecionado: (funcionario: Funcionario | null) => void;
  handleAdicionarDemanda: () => void;
  setTab: (tab: 'paletes' | 'funcionario') => void;
}

export function EtapaDoisAddPalete({ funcionarioSelecionado, setFuncionarioSelecionado, handleAdicionarDemanda, setTab }: EtapaDoisAddPaleteProps) {
              ...
              <div className="max-h-64 overflow-y-auto space-y-1 pr-2">
                {funcionariosFiltrados && funcionariosFiltrados.map((funcionario) => {
                  ...
                  return (
                    <div
                      key={funcionario.id}
                      className={`group flex items-center justify-between p-2.5 border rounded-lg cursor-pointer transition-all duration-200 ${
                        estaSelecionado
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                          estaSelecionado ? 'bg-primary/20 text-primary' : 'bg-primary/10 text-primary'
                        }`}>
